...
class icueConnect:
    def get_available_leds(self):
        ...
        for device_index in range(device_count):
            ...
            self.deviceToLed[device_index] = led_positions
            dev = self.sdk.get_device_info(device_index)
        return leds
    ...
    def getSubDevices(self, device):
        devInfo = self.sdk.get_device_info(device)
        channelDevices = {}
    ...
    def setSubDeviceLeds(self, device, subDevice, RGB_val):
        ...
        if len(devInfo.channels) > 1:
            ...
            cIndex = "C"+str(cIndex)
            for led in self.deviceToLed[device]:
                if cIndex in str(led):
                    self.deviceToLed[device][led] = (RGB_val[0],RGB_val[1],RGB_val[2])
        else:
            for led in self.deviceToLed[device]:
                self.deviceToLed[device][led] = (RGB_val[0],RGB_val[1],RGB_val[2])
        self.sdk.set_led_colors_buffer_by_device_index(device, self.deviceToLed[device])
        self.sdk.set_led_colors_flush_buffer()

    def getDeviceInfo(self,device):
        return self.sdk.get_device_info(device)
    ...
    def perform_pulse_effect(self,wave_duration,RGB_val):
        ...
        while x < 2:
            ...
            for di in range(cnt):
                device_leds = self.all_leds[di]
                for led in device_leds:
                    device_leds[led] = (val_R, val_G, val_B)
                self.sdk.set_led_colors_buffer_by_device_index(di, device_leds)
            self.sdk.set_led_colors_flush_buffer()
    ...
    def __init__(self):
        ...
        if not connected:
            ...
            return
        self.all_leds = self.get_available_leds()
        self.devicesIdMap = self.getDevicesIdMap()
        if not self.all_leds:
            return      
